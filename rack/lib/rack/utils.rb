...
module Rack
  ...
  module Utils
    ...
    class << self
      ...
      attr_accessor :multipart_parser_bytesize_limit
    ...
    def select_best_encoding(available_encodings, accept_encoding)
      ...
      # Only process the first 16 encodings (fixes CVE-2026-34230)
        ...
        if m == "*"
          unless wildcard_seen
            (available_encodings - accept_encoding.map(&:first)).each do |m2|
              expanded_accept_encoding << [m2, q]
            end
            wildcard_seen = true
          end
        else
          expanded_accept_encoding << [m, q]
        end
      end

      encoding_candidates = expanded_accept_encoding.sort { |a, b| b[1] <=> a[1] }.map(&:first)
    ...
    def byte_ranges(env, size, options = {})
      max_ranges = options.fetch(:max_ranges, 100)
